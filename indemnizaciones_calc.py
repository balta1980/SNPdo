...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, obj, obj2, *args, **kwargs):
        ...
        self.listados_id_indem = leew.consulta_lista('worker.db', 'id_indem', 'indemnizaciones', 'id_indem >', '"0"')
        # print(self.listados_id_hora_e)

        # para ordenar y posicionar información
        self.posicion = len(self.listados_id_indem)
    ...
    def manejador_toolbar_nav_izquierda(self):
        if self.probador_de_instancia_para_toolbar():
            if len(self.listados_id_indem) > 0: # verifica que la bd no está vacia
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        self.hubo_modificacion = 0
                        if self.posicion == 0:
                            self.posicion = len(self.listados_id_indem) - 1
                        else:
                            self.posicion = self.posicion - 1
                        self.muestra_historial(self.listados_id_indem[self.posicion])
                else:
                    if self.posicion == 0:
                        self.posicion = len(self.listados_id_indem) - 1
                    else:
    ...
    def manejador_toolbar_nav_derecha(self):
        if len(self.listados_id_indem) > 0:  # verifica que la bd no está vacia
            if self.probador_de_instancia_para_toolbar():
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        if self.posicion >= len(self.listados_id_indem) - 1:
                            self.posicion = 0
                        else:
                            self.posicion = self.posicion + 1
        ...
        if len(self.listados_id_indem) > 0:  # verifica que la bd no está vacia
            if self.probador_de_instancia_para_toolbar():
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        self.posicion = 0
                        self.muestra_historial(self.listados_id_indem[0])
                else:
                    self.posicion = 0
                    self.muestra_historial(self.listados_id_indem[0])
    ...
    def muestra_historial(self, id_indem):
        ...
        self.id_visible.setText(f'{id_indem}')

        # fecha de computo
        data = leew.consulta_gen('worker.db', 'fecha_doc', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if data == None:
        ...
        # nombre trabajador
        data = leew.consulta_gen('worker.db', 'nombre_trab', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if data == None:
            data = 'N/D'
        ...
        data = leew.consulta_gen('worker.db', 'tipo_indem', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if data == None:
            data = 'N/D'
        self.tipo.setCurrentText(data)
        self.tipo.setDisabled(True)


        # nota
        data = leew.consulta_gen('worker.db', 'nota', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if data == None:
        ...
        # monto
        data = leew.consulta_gen('worker.db', 'monto', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if data == None:
        ...
        status_ina = leew.consulta_gen('worker.db', 'computado', 'indemnizaciones', 'id_indem', f'{id_indem}')
        if status_ina == 0:
    ...
    def borrar_cmd(self):
        reply = QtWidgets.QMessageBox.question(self, "Para continuar", "Desea usted borrar la variación al trabajador?"
                                               , QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
        if reply == QtWidgets.QMessageBox.Yes:
            leew.del_gen('worker.db', 'indemnizaciones', 'id_indem', f'{self.id_visible.text()} AND computado = 0')
            self.recarga_en_blanco_nuevo_doc()
            # para ordenar y posicionar información
            self.listados_id_indem = leew.consulta_lista('worker.db', 'id_indem', 'indemnizaciones', 'id_indem >', '"0"')
            self.posicion = len(self.listados_id_indem)
    ...
    def guardar_cmd(self):
        ...
        self.entrada = f'NULL,"{QtCore.QDate.currentDate().toString("dd-MM-yyyy")}","{self.nota.toPlainText()}",' \
                       ...
                       f'"{self.desc_roe}","{self.main_instancia.usuario}", "{self.tipo.currentText()}"'
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            try:
                leew.introduce_gen('worker.db', 'indemnizaciones', self.entrada)
