from list_rem_otros_empleadores import *
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def borrar_cmd(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            try:
                leew.del_gen('worker.db', 'remun_otro_empleador', 'id_roe', f'{self.numero_ficha} AND computado = 0')
                # self.close()
    ...
    def refresh_tabla(self):
        lista_id_otras_remun = leew.consulta_lista('worker.db', 'id_roe', 'remun_otro_empleador', 'computado', '"0"')
        n = len(lista_id_otras_remun)
        ...
        cont = 0
        for id_roe in lista_id_otras_remun:
            id_roe = str(id_roe)
            item = id_roe
            self.tableWidget.setItem(cont, 0, QtWidgets.QTableWidgetItem(item))
            id_trab = leew.consulta_gen('worker.db', 'id', 'remun_otro_empleador', 'id_roe', id_roe)
            ...
            item = leew.consulta_gen('worker.db', 'fecha_doc', 'remun_otro_empleador', 'id_roe', id_roe)
            self.tableWidget.setItem(cont, 2, QtWidgets.QTableWidgetItem(item))
            item = leew.consulta_gen('worker.db', 'monto', 'remun_otro_empleador', 'id_roe', id_roe)
