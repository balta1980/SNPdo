from rem_otros_empleadores import *
import leew
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, obj, obj2, *args, **kwargs):
        ...
        self.hubo_modificacion = 0  # para avisar al usuario que va a perder la información ingresada

        # listado de id de horas extra
        self.listados_id_remun_otros_emp = leew.consulta_lista('worker.db', 'id_roe', 'remun_otro_empleador', 'id_roe >', '"0"')
        # print(self.listados_id_hora_e)

        # para ordenar y posicionar información
        self.posicion = len(self.listados_id_remun_otros_emp)
        ...
        self.nombre_empleador.textChanged.connect(self.set_enable_agregar)
        self.id_empleador.textChanged.connect(self.set_enable_agregar)
    ...
    def manejador_toolbar_nav_izquierda(self):
        if self.probador_de_instancia_para_toolbar():
            if len(self.listados_id_remun_otros_emp) > 0: # verifica que la bd no está vacia
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        self.hubo_modificacion = 0
                        if self.posicion == 0:
                            self.posicion = len(self.listados_id_remun_otros_emp) - 1
                        else:
                            self.posicion = self.posicion - 1
                        self.muestra_historial(self.listados_id_remun_otros_emp[self.posicion])
                else:
                    if self.posicion == 0:
                        self.posicion = len(self.listados_id_remun_otros_emp) - 1
                    else:
    ...
    def manejador_toolbar_nav_derecha(self):
        if len(self.listados_id_remun_otros_emp) > 0:  # verifica que la bd no está vacia
            if self.probador_de_instancia_para_toolbar():
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        if self.posicion >= len(self.listados_id_remun_otros_emp) - 1:
                            self.posicion = 0
                        else:
                            self.posicion = self.posicion + 1
        ...
        if len(self.listados_id_remun_otros_emp) > 0:  # verifica que la bd no está vacia
            if self.probador_de_instancia_para_toolbar():
                if self.hubo_modificacion == 1:
                    if self.advertencia_de_perdida_de_datos() == 1:
                        self.posicion = 0
                        self.muestra_historial(self.listados_id_remun_otros_emp[0])
                else:
                    self.posicion = 0
                    self.muestra_historial(self.listados_id_remun_otros_emp[0])
    ...
    def muestra_historial(self, id_roe):
        ...
        try:  # uso este captyrador de error para que no cargue la data
            ...
            self.nombre_empleador.textChanged.disconnect(self.set_enable_agregar)
            self.id_empleador.textChanged.disconnect(self.set_enable_agregar)
        ...
        self.id_visible.setText(f'{id_roe}')

        # fecha de computo
        data = leew.consulta_gen('worker.db', 'fecha_doc', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
        ...
        # nombre trabajador
        data = leew.consulta_gen('worker.db', 'nombre_trab', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
            data = 'N/D'
        self.trabajador.setCurrentText(data)
        self.trabajador.setDisabled(True)

        # nombre otro empleador
        data = leew.consulta_gen('worker.db', 'nombre_empleador', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
            data = 'N/D'
        self.nombre_empleador.setText(data)
        self.nombre_empleador.setDisabled(True)

        # RNC o CI otro empleador
        data = leew.consulta_gen('worker.db', 'id_empleador', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
            data = 'N/D'
        self.id_empleador.setText(data)
        self.id_empleador.setDisabled(True)


        # nota
        data = leew.consulta_gen('worker.db', 'nota', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
        ...
        # monto
        data = leew.consulta_gen('worker.db', 'monto', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if data == None:
        ...
        status_ina = leew.consulta_gen('worker.db', 'computado', 'remun_otro_empleador', 'id_roe', f'{id_roe}')
        if status_ina == 0:
    ...
    def borrar_cmd(self):
        reply = QtWidgets.QMessageBox.question(self, "Para continuar", "Desea usted borrar la variación al trabajador?"
                                               , QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
        if reply == QtWidgets.QMessageBox.Yes:
            leew.del_gen('worker.db', 'remun_otro_empleador', 'id_roe', f'{self.id_visible.text()} AND computado = 0')
            self.recarga_en_blanco_nuevo_doc()
            # para ordenar y posicionar información
            self.listados_id_remun_otros_emp = leew.consulta_lista('worker.db', 'id_roe', 'remun_otro_empleador', 'id_roe >', '"0"')
            self.posicion = len(self.listados_id_remun_otros_emp)
    ...
    def set_enable_agregar(self):
        if self.nota.toPlainText() != '' and self.monto.value() > 0 and self.nombre_empleador.text() != '' \
                and self.id_empleador.text() != '':
    ...
    def guardar_cmd(self):
        ...
        # print(self.periodo)
        self.desc_roe = "n/d" # no implementado o usado porque las otras remun son auto explicatorias
        ...
        self.entrada = f'NULL,"{QtCore.QDate.currentDate().toString("dd-MM-yyyy")}","{self.nota.toPlainText()}",' \
                       ...
                       f'"{self.desc_roe}","{self.main_instancia.usuario}", "{self.nombre_empleador.text()}", ' \
                       f'"{self.id_empleador.text()}"'
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            try:
                leew.introduce_gen('worker.db', 'remun_otro_empleador', self.entrada)
