...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def borrar_cmd(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            try:
                leew.del_gen('worker.db', 'comisiones', 'id_c', f'{self.numero_ficha} AND computado = 0')
                # self.close()
    ...
    def refresh_tabla(self):
        lista_id_comisiones = leew.consulta_lista('worker.db', 'id_c', 'comisiones', 'computado', '"0"')
        n = len(lista_id_comisiones)
        ...
        cont = 0
        for id_com in lista_id_comisiones:
            id_com = str(id_com)
            item = id_com
            self.tableWidget.setItem(cont, 0, QtWidgets.QTableWidgetItem(item))
            id_trab = leew.consulta_gen('worker.db', 'id', 'comisiones', 'id_c', id_com)
            ...
            item = leew.consulta_gen('worker.db', 'fecha_doc', 'comisiones', 'id_c', id_com)
            self.tableWidget.setItem(cont, 2, QtWidgets.QTableWidgetItem(item))
            item = leew.consulta_gen('worker.db', 'monto', 'comisiones', 'id_c', id_com)
