...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def borrar_cmd(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            try:
                leew.del_gen('worker.db', 'otras_remun', 'idor', f'{self.numero_ficha} AND computado = 0')
                # self.close()
    ...
    def refresh_tabla(self):
        lista_id_otras_remun = leew.consulta_lista('worker.db', 'idor', 'otras_remun', 'computado', '"0"')
        ...
        cont = 0
        for id_or in lista_id_otras_remun:
            id_or = str(id_or)
            item = id_or
            self.tableWidget.setItem(cont, 0, QtWidgets.QTableWidgetItem(item))
            id_trab = leew.consulta_gen('worker.db', 'id', 'otras_remun', 'idor', id_or)
            ...
            item = leew.consulta_gen('worker.db', 'fecha_doc', 'otras_remun', 'idor', id_or)
            self.tableWidget.setItem(cont, 2, QtWidgets.QTableWidgetItem(item))
            item = leew.consulta_gen('worker.db', 'monto', 'otras_remun', 'idor', id_or)
